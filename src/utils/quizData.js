...
const loadQuizData = () => {
  ...
  const quizFiles = ['q1.json', 'q2.json', 'q3.json', 'q4.json', 'q5.json', 'q6.json', 'q7.json'];
      ...
      if (fs.existsSync(filePath)) {
        ...
        const validatedQuestions = questions.map((q, qIndex) => {
          ...
          if (!q.options.every((opt) => opt.fr && opt.en && opt.ar)) {
            console.error(
              `Step: Missing language keys in options for question ${qIndex} of ${filename}`
            );
            throw new Error(`Missing language keys in options for question ${qIndex}`);
          }
          if (!q.correct.fr || !q.correct.en || !q.correct.ar) {
            console.error(
              `Step: Missing language keys in correct answer for question ${qIndex} of ${filename}`
            );
        ...
        console.log(
          `Step: Successfully loaded quiz ${quizName} with ${validatedQuestions.length} questions`
        );
      } else {
