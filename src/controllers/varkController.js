...
const getUserVarkResult = async (req, res) => {
  try {
    ...
    if (!varkTest) {
      return res.status(404).json({ message: 'No VARK results found for this user' });
    }
...
const setUserVarkAnswer = async (req, res) => {
  try {
    const { userId } = req.params;
    const { day, selectedStyle } = req.body;
    ...
    if (uniqueAnswered.size === 21) {
      ...
      response.testCompleted = false;
      response.completedDays = uniqueAnswered.size;
      response.remainingDays = 21 - uniqueAnswered.size;
    }

    await varkTest.save();
    return res.json(response);
  } catch (error) {
    console.error('Error setting user VARK answer:', error);
    res.status(500).json({ message: 'Server error', error });
  }
};
